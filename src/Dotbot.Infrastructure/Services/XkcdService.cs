...
public class XkcdService : IXkcdService
{
    ...
    public async Task<Result<XkcdComicDto>> GetLatestComic() => await GetComic();
    
    public async Task<Result<XkcdComicDto>> GetComic(int? number = null)
    {
        var url = number == null ? LatestUrl : string.Format(ComicUrl, number);
        try
        {
            var comic = await _httpClient.GetFromJsonAsync<XkcdComicDto>(url);
